...
class ClangD(LanguageServer):
    ...
    def __init__(self, config: MultilspyConfig, logger: MultilspyLogger, repository_root_path: str):
        """
        Creates a clangD instance. This class is not meant to be instantiated directly. Use LanguageServer.create() instead.
        """
    ...
    def setup_runtime_dependencies(self, logger: MultilspyLogger, config: MultilspyConfig) -> str:
        ...
        os.chmod(clangd_executable_path, stat.S_IRWXU | stat.S_IRGRP | stat.S_IXGRP | stat.S_IROTH | stat.S_IXOTH)
        return clangd_executable_path

    def _get_initialize_params(self, repository_absolute_path: str) -> InitializeParams:
        """
        Returns the initialization parameters for the language server.
        """
    ...
    @asynccontextmanager
    async def start_server(self) -> AsyncIterator["ClangD"]:
        """
        Starts the C++ Language Server, waits for the server to be ready and yields the LanguageServer instance.
        ...
        async with super().start_server():
            self.logger.log("Starting C++ server process", logging.INFO)
            await self.server.start()
            initialize_params = self._get_initialize_params(os.path.abspath(self.repository_root_path))
            self.logger.log(
